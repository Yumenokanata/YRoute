...
typealias LazyYRoute<S, P, R> = (P) -> YRoute<S, R>

fun <S, P, R> lazyR1(f: (P) -> YRoute<S, R>): LazyYRoute<S, P, R> = f

fun <S, P1, P2, R> lazyR2(f: (P1, P2) -> YRoute<S, R>): LazyYRoute<S, Tuple2<P1, P2>, R> = { f(it.a, it.b) }
...
fun <S, P1, P2, P3, P4, R> lazyR4(f: (P1, P2, P3, P4) -> YRoute<S, R>): LazyYRoute<S, Tuple4<P1, P2, P3, P4>, R> =
    { f(it.a, it.b, it.c, it.d) }


inline fun <S, T, R> route(crossinline f: (S) -> (suspend (Tuple2<RouteCxt, T>) -> Tuple2<S, YResult<R>>)): LazyYRoute<S, T, R> =
    { t -> YRoute { s -> { c -> f(s)(c toT t) } } }

inline fun <S, T, R> routeF(crossinline f: suspend (S, RouteCxt, T) -> Tuple2<S, YResult<R>>): LazyYRoute<S, T, R> =
        { t -> YRoute { s -> { c -> f(s, c, t) } } }

suspend fun <S, T, R> LazyYRoute<S, T, R>.runRoute(state: S, cxt: RouteCxt, t: T): Tuple2<S, YResult<R>> =
    this(t).runRoute(state, cxt)

fun <S, T, R> LazyYRoute<S, T, R>.toAction(param: T): EngineAction<S, T, R> =
    this@toAction toT param

fun <S, T> routeId(): LazyYRoute<S, T, T> = routeF { s, c, t -> s toT Success(t) }

fun <S, T> routeGetState(): LazyYRoute<S, T, S> = routeF { s, _, _ -> s toT Success(s) }

fun <S, T> routeFromSuspend(io: suspend () -> Unit): LazyYRoute<S, T, Unit> =
    routeF { state, _, _ -> state toT Success(io()) }

fun <S, T, R> routeFromF(f: (T) -> R): LazyYRoute<S, T, R> =
    routeF { state, cxt, param -> state toT Success(f(param)) }
...
fun <S : Any, T, R> LazyYRoute<S, T, R>.stateNullable(): LazyYRoute<S?, T, R> =
    routeF { state, cxt, param ->
        if (state == null)
            state toT Fail("State is null, can not get state.")
        else
...
fun <S, T, T1, R> LazyYRoute<S, T, T1>.transform(f: suspend (S, RouteCxt, T1) -> Tuple2<S, YResult<R>>): LazyYRoute<S, T, R> =
    routeF { state, cxt, param ->
        val (newState, resultT1) = this@transform.runRoute(state, cxt, param)
...
fun <S, T, R : Any> LazyYRoute<S, T, R?>.resultNonNull(tag: String = "resultNonNull()"): LazyYRoute<S, T, R> =
    transform { state, cxt, t1 ->
        state toT if (t1 != null) Success(t1) else Fail("Tag $tag | YResult can not be Null.")
    }

fun <S, T, T1, R> LazyYRoute<S, T, T1>.composeWith(f: suspend (S, RouteCxt, T, T1) -> Tuple2<S, YResult<R>>): LazyYRoute<S, T, R> =
    routeF { state, cxt, param ->
        val (newState, resultT1) = this@composeWith.runRoute(state, cxt, param)
...
fun <S, T, R> LazyYRoute<S, T, R>.packageParam(): LazyYRoute<S, T, Tuple2<T, R>> =
    routeF { state, cxt, param ->
        val (newState, result) = this@packageParam.runRoute(state, cxt, param)
        newState toT result.map { r -> param toT r }
    }
...
infix fun <S, T, T1, R> LazyYRoute<S, T, T1>.compose(route2: LazyYRoute<S, T1, R>): LazyYRoute<S, T, R> =
    routeF { state, cxt, param ->
        val (newState, resultT1) = this@compose.runRoute(state, cxt, param)
...
fun <S, T1, T2, R1, R2> stick(route1: LazyYRoute<S, T1, R1>, route2: LazyYRoute<S, T2, R2>): LazyYRoute<S, Coproduct2<T1, T2>, Coproduct2<R1, R2>> =
    routeF { state, cxt, param ->
        when (param) {
            is First  -> route1.runRoute(state, cxt, param.a).let { it.map { result -> result.map { r1 -> First<R1, R2>(r1) } } }
            is Second -> route2.runRoute(state, cxt, param.b).let { it.map { result -> result.map { r2 -> Second<R1, R2>(r2) } } }
        }
    }

suspend fun <S, T1, T2, R1, R2> LazyYRoute<S, Coproduct2<T1, T2>, Coproduct2<R1, R2>>.runRoute21(state: S, cxt: RouteCxt, t1: T1): Tuple2<S, YResult<R1>> =
    runRoute(state, cxt, First<T1, T2>(t1)).let { tuple -> tuple.map { result -> result.map { (it as First).a } } }

suspend fun <S, T1, T2, R1, R2> LazyYRoute<S, Coproduct2<T1, T2>, Coproduct2<R1, R2>>.runRoute22(state: S, cxt: RouteCxt, t2: T2): Tuple2<S, YResult<R2>> =
    runRoute(state, cxt, Second<T1, T2>(t2)).let { tuple -> tuple.map { result -> result.map { (it as Second).b } } }
...
fun <S, T1, T2, T3, R1, R2, R3> stick(route1: LazyYRoute<S, T1, R1>, route2: LazyYRoute<S, T2, R2>, route3: LazyYRoute<S, T3, R3>): LazyYRoute<S, Coproduct3<T1, T2, T3>, Coproduct3<R1, R2, R3>> =
    routeF { state, cxt, param ->
        when (param) {
            is arrow.generic.coproduct3.First  -> route1.runRoute(state, cxt, param.a).let { it.map { result -> result.map { r1 -> arrow.generic.coproduct3.First<R1, R2, R3>(r1) } } }
            is arrow.generic.coproduct3.Second -> route2.runRoute(state, cxt, param.b).let { it.map { result -> result.map { r2 -> arrow.generic.coproduct3.Second<R1, R2, R3>(r2) } } }
            is arrow.generic.coproduct3.Third  -> route3.runRoute(state, cxt, param.c).let { it.map { result -> result.map { r3 -> arrow.generic.coproduct3.Third<R1, R2, R3>(r3) } } }
        }
    }

suspend fun <S, T1, T2, T3, R1, R2, R3> LazyYRoute<S, Coproduct3<T1, T2, T3>, Coproduct3<R1, R2, R3>>.runRoute31(state: S, cxt: RouteCxt, t1: T1): Tuple2<S, YResult<R1>> =
    runRoute(state, cxt, arrow.generic.coproduct3.First<T1, T2, T3>(t1))
        .let { tuple -> tuple.map { result -> result.map { (it as arrow.generic.coproduct3.First).a } } }

suspend fun <S, T1, T2, T3, R1, R2, R3> LazyYRoute<S, Coproduct3<T1, T2, T3>, Coproduct3<R1, R2, R3>>.runRoute32(state: S, cxt: RouteCxt, t2: T2): Tuple2<S, YResult<R2>> =
    runRoute(state, cxt, arrow.generic.coproduct3.Second<T1, T2, T3>(t2))
        .let { tuple -> tuple.map { result -> result.map { (it as arrow.generic.coproduct3.Second).b } } }

suspend fun <S, T1, T2, T3, R1, R2, R3> LazyYRoute<S, Coproduct3<T1, T2, T3>, Coproduct3<R1, R2, R3>>.runRoute33(state: S, cxt: RouteCxt, t3: T3): Tuple2<S, YResult<R3>> =
    runRoute(state, cxt, arrow.generic.coproduct3.Third<T1, T2, T3>(t3))
        .let { tuple -> tuple.map { result -> result.map { (it as arrow.generic.coproduct3.Third).c } } }
...
suspend fun <S, T, R> EngineAction<S, T, R>.startAsync(core: CoreEngine<S>, callback: (YResult<R>) -> Unit): Unit =
    core.runAsync(a(b), callback)

suspend fun <S, T, R> EngineAction<S, T, R>.start(core: CoreEngine<S>): YResult<R> =
    core.run(a(b))

suspend fun <S, R> LazyYRoute<S, Unit, R>.startAsync(core: CoreEngine<S>, callback: (YResult<R>) -> Unit): Unit =
    (this toT Unit).startAsync(core, callback)

suspend fun <S, R> LazyYRoute<S, Unit, R>.start(core: CoreEngine<S>): YResult<R> = (this toT Unit).start(core)
